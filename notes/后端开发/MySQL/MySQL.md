...
索引下推即：当存在索引的列作为判断条件时，MySQL的server层会将这一部分判断条件传递给存储引擎，之后将不符合该索引判断条件的索引项删除，之后拿到符合条件的数据项，再进行回表查询。
...
- 如果没有索引下推，那么根据索引的最左前缀原则，在索引树上只会使用`name`去匹配，之后找到两条记录(id=1, id=4)，再拿这两条记录进行回表查询，**需要回表2次。**

  ![1674388467924](images/1674388467924.png)

- 如果使用索引下推，那么在索引遍历的过程中，会对索引的字段进行判断，将不符合条件的索引项过滤掉，也就是判断`age`是否等于`12`，之后找到一条记录(id=1)，**只需要回表1次。**
